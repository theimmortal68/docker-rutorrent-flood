...
do
    ...
    PUBLIC_IP="73.133.117.62"
    echo $WAN_IP
    echo $PUBLIC_IP
    if [[ $WAN_IP != $PUBLIC_IP ]]; then
        echo "Connection Successful! VPN IP is ${WAN_IP}"
        nginx &
        php-fpm &
        doas -u abc flood -h 0.0.0.0 -p 3000 -d /config/flood --allowedpath /config/flood --allowedpath /downloads --auth none --rthost 127.0.0.1 --rtport 5000 &
        doas -u abc rtorrent -o system.daemon.set=true &
        break
    else
        echo "Connection Unsuccessful. Retrying in 5 seconds ..."
        sleep 5
    fi
done
